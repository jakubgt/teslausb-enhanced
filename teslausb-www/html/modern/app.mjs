...
let healthTimer=null,device=null,files=null,trashView=null,libraryAbort=null,connection=null,stopCardThumbnails=()=>{};
...
async function navigate(page){
  ...
  if(state.page==='recordings'){player.suspend();stopCardThumbnails();}
...
function refreshCardThumbnails(){
  stopCardThumbnails();
  if(state.page==='recordings'&&!document.hidden&&!state.inFlight&&!state.speedTest&&!hasPendingPowerAction())stopCardThumbnails=mountCardThumbnails($('#clip-grid'),currentPage().items,{api});
}
function renderGrid(){
  stopCardThumbnails();
...
async function loadLibrary(day='latest'){
  ...
  state.inFlight=true;stopCardThumbnails();libraryAbort=new AbortController();const saved=player.suspend();updateClipNavigation();const previousDay=state.day;state.day=day;$('#refresh-recordings').disabled=true;$('#recording-day').disabled=true;$('#library-state').textContent='Refreshing the snapshot library…';
  try{
  ...
  finally{state.inFlight=false;refreshCardThumbnails();updateClipNavigation();$('#refresh-recordings').disabled=false;$('#recording-day').disabled=false;refreshHealth();}
}
...
$('#move-confirm').onclick=async()=>{if(moving)return;moving=true;$('#move-confirm').disabled=true;$('#move-cancel').disabled=true;stopCardThumbnails();player.suspend();let moved=0,error=null,ids=[];
...
async function prepareDownload(event,camera){
  ...
  async function prepare(){
    ...
    const paused=player.snapshot();if(state.page==='recordings'){stopCardThumbnails();player.suspend();}
    ...
    finally{if(downloadTasks.has(task))row.querySelector('progress').hidden=true;if(state.page==='recordings'&&!document.hidden&&!state.inFlight&&!state.speedTest&&player.event?.id===paused.id)player.resume();refreshCardThumbnails();}
  }
  await prepare();row.scrollIntoView({block:'nearest'});
}

window.addEventListener('teslausb:pause-media',()=>{stopCardThumbnails();player.suspend();});
window.addEventListener('teslausb:power-state',()=>{connection.setPaused();refreshCardThumbnails();updateClipNavigation();});
window.addEventListener('teslausb:speed-test',e=>{state.speedTest=e.detail?.active===true;$('#refresh-recordings').disabled=state.speedTest||state.inFlight;$('#recording-day').disabled=state.speedTest||state.inFlight;if(state.speedTest)player.suspend();else if(state.page==='recordings'&&!document.hidden&&!state.inFlight)player.resume();refreshCardThumbnails();updateClipNavigation();});
document.addEventListener('visibilitychange',()=>{if(document.hidden){stopCardThumbnails();player.suspend();libraryAbort?.abort();}else if(state.page==='recordings'&&!state.inFlight&&!state.speedTest){player.resume();refreshCardThumbnails();}});
window.addEventListener('pagehide',event=>{stopCardThumbnails();clearInterval(healthTimer);if(!event.persisted)connection.destroy();player.suspend();device?.destroy();device=null;files?.destroy();files=null;trashView?.destroy();trashView=null;libraryAbort?.abort();for(const t of downloadTasks)t.controller?.abort();});
window.addEventListener('pageshow',e=>{if(!e.persisted)return;if(state.page==='recordings'){player.resume();refreshCardThumbnails();}else if(state.page==='device')device=mountDevice($('#device-page'),{api,onNotice:notice});else if(state.page==='files')files=mountFiles($('#files-page'),{api,onNotice:notice,config:state.config});else trashView=mountTrash($('#trash-page'),{api,onNotice:notice,onLibraryChanged:()=>loadLibrary(state.day)});startHealthTimer();});
