...
export class ClipPlayer {
  constructor(container,{api,onDownload,onTrash,onNotice}) {
    ...
    this.event=null;this.position=0;this.camera='front';this.mode='single';this.quality='low';this.rate=1;this.playing=false;this.suspended=false;this.generation=0;this.playIntent=0;this.marker=null;this.metadata=null;this.videos=[];
  ...
  releaseMedia(){this.loadAbort?.abort();clearTimeout(this.previewTimer);for(const video of this.videos){video.pause();video.removeAttribute('src');video.load();}this.videos=[];this.master=null;this.buffering=false;}
  stopPlayback(message){this.playing=false;this.playIntent++;this.videos.forEach(video=>video.pause());this.updateControls();if(message)this.onNotice(message);}
  async playVideo(video,generation,intent=this.playIntent){
    ...
    try{await video.play();if(generation!==this.generation||!this.playing||this.suspended||document.hidden)video.pause();}
    catch{if(generation===this.generation&&intent===this.playIntent&&this.playing&&!this.suspended&&!document.hidden)this.stopPlayback('Playback could not start. Try a single camera or refresh the recording.');}
  }
