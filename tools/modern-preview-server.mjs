...
export async function createPreviewServer({port = 0, media = null} = {}) {
  ...
  const server = http.createServer(async (request,response) => {
    try {
      ...
      if(p.startsWith('/api/v1/actions/'))return json(response,{ok:true,message:'Local fixture action accepted; no device was contacted.'},['/api/v1/actions/reboot','/api/v1/actions/shutdown'].includes(p)?202:200);
