...
async function run() {
  ...
  try {
    ...
    await page.locator('#theme-toggle').click();await waitMedia();await page.getByRole('button',{name:'Play recording',exact:true}).click();await page.getByRole('button',{name:'Pause recording',exact:true}).click();await page.getByRole('button',{name:'All cameras',exact:true}).click();await waitMedia(6);await assertCameraGrid(3,2);await screenshot('recordings-desktop-light');
